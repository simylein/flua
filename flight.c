...
void find_flights(sqlite3 *database, uint8_t (*user_id)[16], char *year, size_t year_len, response_t *response) {
	...
	while (1) {
		...
		if (result == SQLITE_ROW) {
			...
			append_body(response, &n_leading_bytes, sizeof(leading_bytes) - 1);
			append_body(response, ((uint8_t *)&n_starts_at) + (sizeof(n_starts_at) - starts_at_bytes), starts_at_bytes);
			append_body(response, ((uint8_t *)&n_ends_at) + (sizeof(n_ends_at) - ends_at_bytes), ends_at_bytes);
			for (uint8_t index = 0; index < 5; index++) {
				append_body(response, ((uint8_t *)&(*altitude_bins)[index]) + (sizeof(*altitude_bins[0]) - altitude_bins_bytes[index]),
										altitude_bins_bytes[index]);
			}
			append_body(response, ((uint8_t *)&n_altitude_min) + (sizeof(n_altitude_min) - altitude_min_bytes), altitude_min_bytes);
			append_body(response, ((uint8_t *)&n_altitude_max) + (sizeof(n_altitude_max) - altitude_max_bytes), altitude_max_bytes);
			for (uint8_t index = 0; index < 5; index++) {
				append_body(response, ((uint8_t *)&(*thermal_bins)[index]) + (sizeof(*thermal_bins[0]) - thermal_bins_bytes[index]),
										thermal_bins_bytes[index]);
			}
			append_body(response, &max_climb, sizeof(max_climb));
			append_body(response, &max_sink, sizeof(max_sink));
			for (uint8_t index = 0; index < 5; index++) {
			...
			append_body(response, ((uint8_t *)&n_distance_flown) + (sizeof(n_distance_flown) - distance_flown_bytes),
